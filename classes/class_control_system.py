...
class ControlSystem:
    def __init__(self, name):
        self.name = name
        self.global_symbols: Dict[str,SymbolModel] = None
        self.global_constants: Dict[str, ConstantModel] = None
    ...
    def add_global_constant(
        ...
        self.global_constants[name] = {
            ...
            "is_global": True}   
        for tf_name, tf_instance in self.components.tfs.items():
            # Add symbol (FIXED: use dict assignment, not append)
            tf_instance.symbols[name] = symbol
            
            # Add constant
            tf_instance.constants[name] = self.global_constants[name]
            
            # Optional: Trigger recalculation if TF already defined
            if tf_instance.symbolic is not None:
                tf_instance.numeric = tf_instance.symbolic.subs(
                    tf_instance.get_constant_values()
                )
    ...
    def edit_global_constant(self, name: str, value: float, description='None', unit='-',is_global=True):
        if name in self.global_constants:
            ...
            symbol = utils_sympy.add_symbol(name, is_real=True, is_positive=True)
    ...
    def impulse(self, *tfs: str, delay_times: List[float] = None, sweep_params: Dict[str, List[float]] = None):    
        ...
        if not sweep_params or all(not values for values in sweep_params.values()):
            ...
            labels = list(tfs)
            utils_plots.plot_response(*responses, labels=labels)
            return
        
        responses_list, labels_list = utils_tf.sweep_impulse_responses(self,tf_instances=tf_instances,delay_times=delay_times,sweep_params=sweep_params,is_global=True)
        utils_plots.plot_response(*responses_list, labels=labels_list)
    ...
    def step(self, *tfs: str, delay_times: List[float] = None, sweep_params: Dict[str, List[float]] = None):    
        ...
        if not sweep_params or all(not values for values in sweep_params.values()):
            ...
            labels = list(tfs)
            utils_plots.plot_response(*step_responses, labels=labels)
            return
        
        responses_list, labels_list = utils_tf.sweep_step_responses(self,tf_instances=tf_instances,delay_times=delay_times,sweep_params=sweep_params,is_global=True)
        utils_plots.plot_response(*responses_list, labels=labels_list)
    ...
    def ramp(self, *tfs: str, t_range: Tuple[float, float] = (0, 10), n_points: int = 1000, delay_times: List[float] = None, sweep_params: Dict[str, List[float]] = None):    
        ...
        if not sweep_params or all(not values for values in sweep_params.values()):
            ...
            labels = list(tfs)
            utils_plots.ramp(*tf_numerics, t_range=t_range, n_points=n_points, delay_times=delay_times, labels=labels)
            return
        
        tf_numerics_list, delay_times_list, labels_list = utils_tf.sweep_tfs(self,tf_instances=tf_instances,delay_times=delay_times,sweep_params=sweep_params,is_global=True)

        utils_plots.ramp(*tf_numerics_list, t_range=t_range, n_points=n_points, delay_times=delay_times_list, labels=labels_list)
    ...
    def bode(self, *tfs: str, w_range: Tuple[float, float] = (0.1, 100), n_points: int = 1000, sweep_params: Dict[str, List[float]] = None):    
        ...
        if not sweep_params or all(not values for values in sweep_params.values()):
            tf_numerics = [tf_instance.tf.numeric for tf_instance in tf_instances]
            labels = list(tfs)
            utils_plots.bode(*tf_numerics, w_range=w_range, n_points=n_points, labels=labels)
            return
        
        tf_numerics_list, delay_times_list, labels_list = utils_tf.sweep_tfs(self,tf_instances=tf_instances,delay_times=None,sweep_params=sweep_params,is_global=True)
        
        utils_plots.bode(*tf_numerics_list, w_range=w_range, n_points=n_points, labels=labels_list)
